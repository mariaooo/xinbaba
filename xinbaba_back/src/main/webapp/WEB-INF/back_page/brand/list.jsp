...
<div class="box-positon">
    ...
    <form class="ropt">
        <input class="add" type="button" value="添加"
               onclick="javascript:window.location.href='${pageContext.request.contextPath}/view/brand/add.do'"/>
    </form>
    <div class="clear"></div>
</div>
<div class="body-box">
    <form action="${pageContext.request.contextPath}/brand/list.do" method="post" style="padding-top:5px;">
        ...
        <select name="isDisplay">
            <option value="1"
                    <c:if test="${isDisplay == 1}">selected</c:if> >是
            </option>
            <option value="0" <c:if test="${isDisplay == 0}">selected</c:if>>不是</option>
        </select>
        <input type="submit" class="query" value="查询"/>
    </form>
    <form id="brandListForm" method="post">
        <table cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
            <thead class="pn-lthead">
            <tr>
                <th width="20"><input type="checkbox" id="brandCheckAll" onclick="allCheckBox('ids',this.checked)"/>
                </th>
                <th>品牌ID</th>
                <th>品牌名称</th>
                <th>品牌图片</th>
            ...
            <c:forEach items="${page.list}" var="brand">
                <tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
                    <td><input type="checkbox" value="${brand.id}" name="ids" onchange="checkBox(name, this.checked)"/>
                    </td>
	...
	<span class="r inb_a page_b">
        ...
        <c:forEach begin="${pageNo - 4 <= 1 ? 1: pageNo - 4}"
                   end="${pageNo + 4 >page.pageTotal ? page.pageTotal : pageNo + 4 }" step="1" var="n">
            <c:if test="${pageNo == n}">
		...
		共<var>${page.pageTotal}</var>页 到第  <input type="number" min="1" max="${page.pageTotal}" size="3" id="PAGENO"
    ...
    <div style="margin-top:15px;"><input class="del-button" type="button" value="删除"
                                         onclick="optDelete('${pageNo}','${isDisplay}','${name}');"/></div>
</div>
<script type="text/javascript">
    ...
    function allCheckBox(name, status) {
        $("input[name='" + name + "']").attr('checked', status);
    }
    function checkBox(name, status) {
        const check = Array.from(document.getElementsByName(name)).every(function (e) {
            return e.checked;
        });
        $('#brandCheckAll').attr('checked', check);
    }
    function optDelete(pageNo, isDisplay, name) {
        const s = $("input[name='ids']:checked").size();
        if (s < 1) {
            alert('you should choose one at least!');
        } else {
            if (confirm("are you sure delete?")) {
                $("#brandListForm").attr('action', '${pageContext.request.contextPath}/brand/delete.do?name=' + name + '&pageNo=' + pageNo + '&isDisplay=' + isDisplay);
